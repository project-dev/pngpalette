...
import { inflate, deflate } from '../node_modules/zlib.es/dist/esm/zlib.js';
 ...
 */
export type PNGType = {
    IHDR:{[key:string]:any}|undefined;
    PLTE:Uint8Array|undefined;
    IDAT:Uint8Array|undefined;
    tRNS:Uint8Array|undefined;
    gAMA:number|undefined;
    cHRM:{[key:string]:any}|undefined;
    sRGB:number|undefined;
    iCCP:{[key:string]:any}|undefined;
    tEXt:{[key:string]:any}|undefined;
    zTXt:{[key:string]:any}|undefined;
    iTXt:{[key:string]:any}|undefined;
    bKGD:Uint8Array | number|undefined;
    pHYs:{[key:string]:any}|undefined;
    sBIT:{[key:string]:any}|undefined;
    sPLT:{[key:string]:any}|undefined;
    hIST:Uint8Array|undefined;
    tIME:{[key:string]:any}|undefined;
    fRAc:Uint8Array|undefined;
    gIFg:Uint8Array|undefined;
    gIFt:Uint8Array|undefined;
    gIFx:Uint8Array|undefined;
    oFFs:Uint8Array|undefined;
    pCAL:Uint8Array|undefined;
    sCAL:Uint8Array|undefined;
}
...
export class PNG {
     ...
     * PNGファイルシグネチャ 
     */
    public static PNG_SIGNATURE : Uint8Array = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);

    /** 
     * PLTEのサイズ 
     * 非推奨：使わないように修正予定
     */
     public static PLT_LENGTH = 0x0300;

    /** 
     * パレットのChunk Typeの位置 
     * 非推奨：使わないように修正予定
     */
     public static PALETTE_CHUNK_TYPE_POS = 37;

    /** 
     * パレットデータの位置 
     * 非推奨：使わないように修正予定
     */
    public static PALETTE_DATA_POS = 41;

    public PNGData! : PNGType;
     ...
     * @memberof PNG
     */
    public buff : Uint8Array | undefined;
     ...
     */
    public static async load(url : String) : Promise<PNG>{
        let png = new PNG();
        let data = await fetch(url.toString());
        let buffBase = await data.arrayBuffer();
    ...
    private static anProcMap : {[key : string] : Function} = {
        "IHDR":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            //イメージヘッダ
            // IHDRチャンク                   25byte 
        ...
        "PLTE":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            //パレット
            imgData.PNGData.PLTE = buff;
        },
        "IDAT":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            //イメージデータ
        ...
        "tRNS":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            //透明度
            /*
            switch(colorType){
                case 0:
        ...
        "gAMA":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            //イメージガンマ
            console.warn("!!" + chunkType +" no test!!");
            let pos = 0;
            imgData.PNGData.gAMA = buffToNum(buff.slice(pos, pos+4), 4);
        },
        "cHRM":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            //基礎色度
            console.warn("!!" + chunkType +" no test!!");
        ...
        "sRGB":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // 標準 RGB カラースペース
            console.warn("!!" + chunkType +" no test!!");
            imgData.PNGData.sRGB = buff[0];
        },
        "iCCP":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // 組み込み ICC プロフィール
            console.warn("!!" + chunkType +" no test!!");
            var info : string = "";
            for(var i = 2; i < len; i++){
        ...
        "tEXt":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // テキストデータ
            console.warn("!!" + chunkType +" no test!!");
            var info : string = "";
            for(var i = 2; i < len; i++){
        ...
        "zTXt":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // 圧縮されたテキストデータ
        ...
        "iTXt":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // 国際的なテキストデータ
            console.warn("!!" + chunkType +" unimplemented!!");
            var iTxt = {
                //圧縮されて居おるかどうか
        ...
        "bKGD":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
        ...
        "pHYs":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // 物理的なピクセル寸法
            console.warn("!!" + chunkType +" no test!!");
            try{
        ...
        "sBIT":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // 有効なビット
            console.warn("!!" + chunkType +" no test!!");
            var colorType = imgData.PNGData.IHDR!["color"];
            try{
        ...
        "sPLT":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // 推奨パレット
            console.warn("!!" + chunkType +" unimplemented!!");
            // サンプル精度
        ...
        "hIST":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // パレットヒストグラム
            console.warn("!!" + chunkType +" unimplemented!!");
            // ここからの処理は要調査
            //imgData.hIST;
        },
        "tIME":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // イメージの最終更新時間
            console.warn("!!" + chunkType +" no test!!");
        ...
        "fRAc":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // フラクタル・イメージ・パラメータ
            console.warn("!!" + chunkType +" unimplemented!!");
        },
        "gIFg":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // GIFグラフィック制御拡張
            console.warn("!!" + chunkType +" unimplemented!!");
        },
        "gIFt":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // GIFプレーン・テキスト拡張
            console.warn("!!" + chunkType +" unimplemented!!");
        },
        "gIFx":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // GIF応用拡張
            console.warn("!!" + chunkType +" unimplemented!!");
        },
        "oFFs":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // イメージ・オフセット
            console.warn("!!" + chunkType +" unimplemented!!");
        },
        "pCAL":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // 画素値の較正
            console.warn("!!" + chunkType +" unimplemented!!");
        },
        "sCAL":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // イメージの物理寸法
            console.warn("!!" + chunkType +" unimplemented!!");
        },
        "IEND":function(chunkType : string, buff : Uint8Array, len : number, imgData : PNG){
            // イメージ終端
        }
    };
